...
combinefunctionsaic <- function(xlist, ylist, bsampleslist, functions, masklist, pars, verbose = TRUE, cdf=TRUE) {
...
#~             print(summary(tmp))
    ...
    return(list(fitresults=fitresults, 
                savefits=savefits,
                weights=weights,
                weighted=list(mean=wmean, sd=sd(wbootstraps, na.rm=T), sdcomb=werr, bootsamples=wbootstraps)
        ))
    }
...
plotlistfits <- function(fitlist, xlim=c(NA), ylim=c(NA), cols=seq(1, length(fitlist)), pchs=rep(1, length(fitlist)), 
    ...
    if (is.na(ylim[1])) ylim <- minmax(c(unlist(sapply(X=fitlist, FUN=getElement, name="y"), use.names=F) + unlist(sapply(X=fitlist, FUN=getElement, name="dy"), use.names=F),
                                         unlist(sapply(X=fitlist, FUN=getElement, name="y"), use.names=F) - unlist(sapply(X=fitlist, FUN=getElement, name="dy"), use.names=F)))
    if(drawlegend) {
    ...
    xseq <- seq(from=xlim[1]*(1-2*sign(xlim[1])), to=xlim[2]*(1+0.5*sign(xlim[2])), length.out=200)
    for (i in seq(1, length(fitlist))) {
        if (fitlist[[i]]$errormodel == "yerrors") {
          plotwitherror(x = fitlist[[i]]$x, y = fitlist[[i]]$y,
                dy = fitlist[[i]]$dy, rep=TRUE, col=cols[i], pch=pchs[i], ...)
        } else {
          plotwitherror(x = fitlist[[i]]$x, y = fitlist[[i]]$y,
                dy = fitlist[[i]]$dy, dx = fitlist[[i]]$dx, rep=TRUE, col=cols[i], pch=pchs[i], ...)
        }
