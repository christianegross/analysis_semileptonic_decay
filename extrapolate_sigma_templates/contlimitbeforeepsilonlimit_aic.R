...
#source("/hiskp4/gross/heavymesons/helpscripts/aichelpers.R")
...
determineDGDq2_contlim_aic <- function(resultpathlist, filenames, th,
...
for (iset in isets) {
    mymasks <- list(rep(NA, 4))
    mymasks <- rep(mymasks, maxz)
    ...
    for (iz in zlist) {
        for(icomb in comblist) {
            ...
            if ("stat" %in% errors) {
                ...
                rep(list(afm^2), length(fitfnlist))
                fitresult <- try(combinefunctionsaic(xlist=rep(list(afm^2), length(fitfnlist)), ylist=rep(list(DGamma), length(fitfnlist)),
                                    bsamples=rep(list(bs), length(fitfnlist)), functions=fitfnlist, pars=par.guesslist,
                                    mask=rep(list(mymasks[[iz+1]]), length(fitfnlist)), cdf=F))
                
                if(!inherits(fitresult, "try-error")) {
                if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title, "only stat"), xlab="a[fm^2]", ylab="DG/Dw2", cols=cols, drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
                if (doplot) par(lwd=3)
                if (doplot) try(plotwitherror(x=0, y=fitresult$weighted$mean, dy=fitresult$weighted$sd, rep=TRUE, col="red", cex=1))
                if (doplot) par(lwd=1)
                ...
                result <- rbind(result, data.frame(w=datalist[[datanumber]][[2+iset]]$metadata$w,
                                            ...
                                            DGDq2=fitresult$weighted$mean, dDGDq2=fitresult$weighted$sd,
                                            errtype = "stat"))
                ...
                resultdat$DGDq2[4*parindex-3]      <- fitresult$weighted$mean
                resultdat$dDGDq2[4*parindex-3]     <- fitresult$weighted$sd
                resultdat$dat[, 4*parindex-3]      <- fitresult$weighted$bootsamples
...
#~ #                print(dDGamma_sys/dDGamma)
                ...
                ## extrapolate a^2 to zero
                fitresult <- try(combinefunctionsaic(xlist=rep(list(afm^2), length(fitfnlist)), ylist=rep(list(DGamma), length(fitfnlist)),
                                    bsamples=rep(list(bootsys), length(fitfnlist)), functions=fitfnlist, pars=par.guesslist,
                                    mask=rep(list(mymasks[[iz+1]]), length(fitfnlist)), cdf=F))
                
                if(!inherits(fitresult, "try-error")) {
                if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title, "stat + sys"), xlab="a[fm^2]", ylab="DG/Dw2", cols=cols, drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
                if (doplot) par(lwd=3)
                if (doplot) try(plotwitherror(x=0, y=fitresult$weighted$mean, dy=fitresult$weighted$sd, rep=TRUE, col="red", cex=1))
                if (doplot) par(lwd=1)
                ...
                resultdat$DGDq2[4*parindex-2]      <- fitresult$weighted$mean
                resultdat$dDGDq2[4*parindex-2]     <- fitresult$weighted$sd
                resultdat$dat[, 4*parindex-2]      <- fitresult$weighted$bootsamples
            ...
            if ("vol" %in% errors) {
                ...
                fitresult <- try(combinefunctionsaic(xlist=rep(list(afm^2), length(fitfnlist)), ylist=rep(list(DGamma), length(fitfnlist)),
                                    bsamples=rep(list(bootvol), length(fitfnlist)), functions=fitfnlist, pars=par.guesslist,
                                    mask=rep(list(mymasks[[iz+1]]), length(fitfnlist)), cdf=F))
                
                if(!inherits(fitresult, "try-error")) {
                if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title, "stat + vol"), xlab="a[fm^2]", ylab="DG/Dw2", cols=cols, drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
                if (doplot) par(lwd=3)
                if (doplot) try(plotwitherror(x=0, y=fitresult$weighted$mean, dy=fitresult$weighted$sd, rep=TRUE, col="red", cex=1))
                if (doplot) par(lwd=1)
                ...
                resultdat$DGDq2[4*parindex-1]      <- fitresult$weighted$mean
                resultdat$dDGDq2[4*parindex-1]     <- fitresult$weighted$sd
                resultdat$dat[, 4*parindex-1]      <- fitresult$weighted$bootsamples
            ...
            if ("tot" %in% errors) {
                ...
                fitresult <- try(combinefunctionsaic(xlist=rep(list(afm^2), length(fitfnlist)), ylist=rep(list(DGamma), length(fitfnlist)),
                                    bsamples=rep(list(boottot), length(fitfnlist)), functions=fitfnlist, pars=par.guesslist,
                                    mask=rep(list(mymasks[[iz+1]]), length(fitfnlist)), cdf=F))
                
                if(!inherits(fitresult, "try-error")) {
                if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title, "stat + sys + vol"), xlab="a[fm^2]", ylab="DG/Dw2", cols=cols, drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
                if (doplot) par(lwd=3)
                if (doplot) try(plotwitherror(x=0, y=fitresult$weighted$mean, dy=fitresult$weighted$sd, rep=TRUE, col="red", cex=1))
                if (doplot) par(lwd=1)
                ...
                resultdat$DGDq2[4*parindex]      <- fitresult$weighted$mean
                resultdat$dDGDq2[4*parindex]     <- fitresult$weighted$sd
                resultdat$dat[, 4*parindex]      <- fitresult$weighted$bootsamples
