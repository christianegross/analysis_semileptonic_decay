...
plot_stability_lambda <- function(path, nset=1, neps=22, nnorm=3, inputfile="") {
    for (iset in seq(0, (nset-1))) {
        for (icomb in seq(0, 4)) {
            for (iz in seq(0, 2)) {
                for (ieps in seq(0, (neps-1))) {
                    filename <- sprintf("%s/DG_%d_iset_%d_ieps_%d_icomb_%d.dat", path, iz, iset, ieps, icomb)
...
plot_stability_AA0 <- function(path, nset=1, neps=22, nnorm=3, inputfile="") {
    for (iset in seq(0, (nset-1))) {
        for (icomb in seq(0, 4)) {
            for (iz in seq(0, 2)) {
                for (ieps in seq(0, (neps-1))) {
...
plot_reconstruction <- function(path, nset=1, neps=22, nnorm=3, plotnorm=c(-1), inputfile="") {
