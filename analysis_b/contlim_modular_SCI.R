...
B64 <- readRDS(sprintf("%s/B64/analyse/tables/%s_SCI_sigma_%s_m%d_%s.RDS", opt$folder, opt$mode, opt$kernel, opt$bmass, opt$error))
ntheta <- dim(B64)[2]
nboot <- dim(B64)[3]

C80 <- readRDS(sprintf("%s/C80/analyse/tables/%s_SCI_sigma_%s_m%d_%s.RDS", opt$folder, opt$mode, opt$kernel, opt$bmass, opt$error))
stopifnot(ntheta == dim(C80)[2])
stopifnot(nboot == dim(C80)[3])

D96 <- readRDS(sprintf("%s/D96/analyse/tables/%s_SCI_sigma_%s_m%d_%s.RDS", opt$folder, opt$mode, opt$kernel, opt$bmass, opt$error))
...
C80_table <- C80_table[C80_table$kernel==opt$kernel, ]
D96_table <- D96_table[D96_table$kernel==opt$kernel, ]
...
restable <- data.frame(iz=c(), theta=c(), DG=c(), dDG=c(), dsys=c(), dtot=c(), pull=c(), cutoff=c(), extrapolation=c())
...
for(theta in thetas) {
  for(iz in 0:zmax) {
    ...
    plot(fit1, 
         xlab="a^2[fm^2]", ylab="", main=paste("Z", iz, "theta", theta), 
    ...
    reslist[[paste0("iz", iz, "theta", theta)]] <- res
    ...
    reslistsys[[paste0("iz", iz, "theta", theta)]] <- res
    ...
    restable <- rbind(restable, data.frame(iz=iz, theta=theta, DG=average, dDG=averagese, dsys=dsys, dtot=dtot, pull=pull, cutoff=(average-fit1$y[3])/average, extrapolation="aic"))
    restable <- rbind(restable, data.frame(iz=iz, theta=theta, DG=fit1$t0[1], dDG=fit1$se[1], dsys=dsyslin, dtot=sqrt(fit1$se[1]^2+dsyslin^2), pull=pulllin, cutoff=(fit1$t0[1]-fit1$y[3])/fit1$t0[1], extrapolation="linear"))
  }
}

saveRDS(object=reslist, file=sprintf("%s/%s_SCI_contlimit_%s_m%d_%s_AIC.RDS", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, opt$error))
saveRDS(object=reslistlinear, file=sprintf("%s/%s_SCI_contlimit_%s_m%d_%s_linear.RDS", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, opt$error))
saveRDS(object=reslistsys, file=sprintf("%s/%s_SCI_contlimit_%s_m%d_%s_cont_AIC.RDS", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, opt$error))
saveRDS(object=reslistlinearsys, file=sprintf("%s/%s_SCI_contlimit_%s_m%d_%s_cont_linear.RDS", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, opt$error))
write.table(restable, file=sprintf("%s/%s_SCI_contlimit_%s_m%d_%s_cont.csv", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, opt$error))
