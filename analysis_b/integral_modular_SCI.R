...
## save results
write.table(resint, sprintf("%s/%s_SCI_integral_%s_m%d_%s.csv", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, errstring), row.names=F)
saveRDS(bsint, sprintf("%s/%s_SCI_integral_%s_m%d_%s.RDS", opt$plotfolder, opt$mode, opt$kernel, opt$bmass, errstring))
