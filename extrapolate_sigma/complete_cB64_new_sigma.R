...
pdf(sprintf("complete_cB64_new_sigma%s.pdf", comment), title="")
...
#~ result <- data.frame(tsnk=NA, Nt=NA, w=NA, nerr=NA, iz=NA, icomb=NA, DGDq2=NA, dDGDq2=NA, chi=NA, p=NA, includesys=NA, fitcorr=NA)
...
files <- c("th1/new_sigma/outputDGammaDq2/DGammaDq2.bin",
           ...
           "th5/new_sigma/outputDGammaDq2/DGammaDq2.bin",
            "th6_t56_44/new_sigma/outputDGammaDq2/DGammaDq2.bin",
...
th <- c(1, 3, 5, 6, 7, 9, 2, 4, 8, 9.5)
nerr <- c(2, 2, 2, 2, 2, 2, 2, 2, 2, 2)

#~ for (index in seq(1, length(files))){
#~     data <- read_in_DGDq2(filename=files[index], write=FALSE, resultpath="/home/gross/Documents/heavymesons/data/cB211ab.07.64/")
...
savename <- sprintf("complete_cB64_new_sigma%s", comment)

par.guess <- rep(1, 5)


determineDGDq2(resultpath = "/home/gross/Documents/heavymesons/data/cB211ab.07.64/", filenames = files, 
...
savename <- sprintf("complete_cB64_new_sigma%s.csv", comment)
result <- read.table(savename, header=TRUE)
pdf(sprintf("complete_cB64_DqDq2_new_sigma%s.pdf", comment), title="")
...
ylim=c(0, max(result$DGDq2[mask] + result$dDGDq2[mask])), xlim=c(0, max(result$q[mask]^2)))

plotwitherror(x=result$q[mask]^2, y=result$dDGDq2[mask],
main="errors differential decay rate for tsink=56, tins=44, including systematical error", xlab="q^2 [GeV^2]", 
ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]", xlim=c(0, max(result$q[mask]^2)))

plotwitherror(x=result$q[mask & result$dDGDq2 < 1]^2, y=result$dDGDq2[mask & result$dDGDq2 < 1],
main="errors differential decay rate for tsink=56, tins=44, including systematical error", xlab="q^2 [GeV^2]", 
ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]", xlim=c(0, max(result$q[mask]^2)))

plotwitherror(x=result$q[mask]^2, y=result$dDGDq2[mask] / abs(result$DGDq2[mask]),
main="rel. errors differential decay rate for tsink=56, tins=44, including systematical error", xlab="q^2 [GeV^2]", 
ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]", xlim=c(0, max(result$q[mask]^2)))
...
plotwitherror(x=result$q[mask & result$dDGDq2 < 1]^2, y=result$dDGDq2[mask & result$dDGDq2 < 1],
main="errors differential decay rate for tsink=56, tins=44, excluding systematical error", xlab="q^2 [GeV^2]", 
ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]", xlim=c(0, max(result$q[mask]^2)))
...
plotwitherror(x=result$q[mask]^2, y=result$dDGDq2[mask] / abs(result$DGDq2[mask]),
main="rel. errors differential decay rate for tsink=56, tins=44, excluding systematical error", xlab="q^2 [GeV^2]", 
ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]", xlim=c(0, max(result$q[mask]^2)))
...
write.table(result, sprintf("complete_cB64_converted_new_sigma%s.csv", comment))
