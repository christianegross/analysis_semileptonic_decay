...
pdf(sprintf("complete_cC80_new_sigma%s.pdf", comment), title="")
...
tsnk <- c(65, 65, 65, 65, 65, 65, 65, 65, 65, 65)
Nt <- c(37, 37, 37, 37, 37, 37, 37, 37, 37, 37)
th <- c(1, 2, 3, 4, 5, 6, 7, 8, 9, 9.5)
nerr <- c(2, 2, 2, 2, 2, 2, 2, 2, 2, 2)

savename <- sprintf("complete_cC80_new_sigma%s", comment)
par.guess <- rep(1, 5)

determineDGDq2(resultpath = "/home/gross/Documents/heavymesons/data/cC211.06.80/", filenames = files, 
...
if(TRUE) {
a <- 0.06821 / 0.1973269804 # fm / hbarc = GeV^-1
am_H <- 6.839642e-01 # dimensionless
...
savename <- sprintf("complete_cC80_new_sigma%s.csv", comment)
result <- read.table(savename, header=TRUE)
result <- na.omit(result)
pdf(sprintf("complete_cC80_DqDq2_new_sigma%s.pdf", comment), title="")
...
write.table(result, sprintf("complete_cC80_converted_new_sigma%s.csv", comment))
