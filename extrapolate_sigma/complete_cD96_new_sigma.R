...
pdf(sprintf("complete_cD96_new_sigma%s.pdf", comment), title="")
...
tsnk <- c(78, 78, 78, 78, 78, 78, 78, 78, 78, 78)
Nt <- c(46, 46, 46, 46, 46, 46, 46, 46, 46, 46)
th <- c(1, 3, 5, 7, 9, 2, 4, 6, 8, 9.5)
nerr <- c(2, 2, 2, 2, 2, 2, 2, 2, 2, 2)

savename <- sprintf("complete_cD96_new_sigma%s", comment)
par.guess <- rep(1, 5)

determineDGDq2(resultpath = "/home/gross/Documents/heavymesons/data/cD211.054.96/", filenames = files, 
...
if(TRUE) {
a <- 0.05692/ 0.1973269804 # fm / hbarc = GeV^-1
am_H <- 5.696513e-01 # dimensionless

conversionfactor <- (am_H/a)^3/2/pi

print(conversionfactor)

savename <- sprintf("complete_cD96_new_sigma%s.csv", comment)
result <- read.table(savename, header=TRUE)
pdf(sprintf("complete_cD96_DqDq2_new_sigma%s.pdf", comment), title="")
...
plotwitherror(x=result$q[mask]^2, y=result$DGDq2[mask], dy=result$dDGDq2[mask],
main="differential decay rate for tsink=78, tins=62, including systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 DGamma / Dq^2 [GeV^-3]",
ylim=c(0, max(result$DGDq2[mask] + result$dDGDq2[mask])))

#~ plotwitherror(x=result$q[mask & result$Nt==46]^2, y=result$dDGDq2[mask & result$Nt==46],
#~ main="errors differential decay rate for tsink=78, tins=62, including systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]")
#~ plotwitherror(x=result$q[mask & result$Nt==31]^2, y=result$dDGDq2[mask & result$Nt==31],
#~ rep=TRUE, col=2, pch=2)
#~ legend(x="topleft", legend=c("Nt 46", "Nt 31"), col=c(1, 2), pch=c(1, 2))

#~ plotwitherror(x=result$q[mask & result$Nt==46 & result$dDGDq2 < 1]^2, y=result$dDGDq2[mask & result$Nt==46 & result$dDGDq2 < 1],
#~ main="errors differential decay rate for tsink=78, tins=62, including systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]")
#~ legend(x="topleft", legend=c("Nt 46"), col=c(1), pch=c(1))

#~ plotwitherror(x=result$q[mask & result$Nt==46]^2, y=result$dDGDq2[mask & result$Nt==46] / abs(result$DGDq2[mask & result$Nt==46]),
#~ main="rel. errors differential decay rate for tsink=78, tins=62, including systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]")
#~ legend(x="topleft", legend=c("Nt 46"), col=c(1), pch=c(1))
...
plotwitherror(x=result$q[mask]^2, y=result$DGDq2[mask], dy=result$dDGDq2[mask],
main="differential decay rate for tsink=78, tins=62, excluding systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 DGamma / Dq^2 [GeV^-3]",
ylim=c(0, max(result$DGDq2[mask] + result$dDGDq2[mask])))
#~ plotwitherror(x=result$q[mask & result$Nt==46 & result$dDGDq2 < 1]^2, y=result$dDGDq2[mask & result$Nt==46 & result$dDGDq2 < 1],
#~ main="errors differential decay rate for tsink=78, tins=62, excluding systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]")
#~ legend(x="topleft", legend=c("Nt 46"), col=c(1), pch=c(1))
...
#~ plotwitherror(x=result$q[mask & result$Nt==46]^2, y=result$dDGDq2[mask & result$Nt==46] / abs(result$DGDq2[mask & result$Nt==46]),
#~ main="rel. errors differential decay rate for tsink=78, tins=62, excluding systematical error", xlab="q^2 [GeV^2]", ylab="24 pi^3 d(DGamma / Dq^2) [GeV^-3]")
#~ legend(x="topleft", legend=c("Nt 46"), col=c(1), pch=c(1))
...
write.table(result, sprintf("complete_cD96_converted_new_sigma%s.csv", comment))
