...
ensembles <- c("cB211.07.64", "cB211.07.96", "cC211.06.80", "cD211.054.96", "cE211.044.112", "cB211.07.48_300", "cB211.07.48_400", "cB211.07.64_48_36")
...
for(kernel in c("sigmoid", "erf")) {
  for(channel in c("cd", "cs")) {
    ...
    files <- c(sprintf("th1/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th2/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th3/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th4/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th5/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th6/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th7/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th8/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th9/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel),
               sprintf("th9.5/%s_new/outputDMammaDq2/DMammaDq2.bin", kernel))
    ...
    for(ens_index in seq_along(ensembles)) {
      ...
      determineDMDq2_all(resultpath = sprintf("/hiskp4/gross/heavymesons/data/%s/%s/", channel, ens), filenames = files, 
                         tsnk = tsnk, Nt = Nt, th = th, nerr = nerr, amin = maxAmin, savename = savename,
                         fitfn = fitfn, par.guess = par.guess, errors=c("stat", "sys", "vol", "tot"),
                         volumetable=sprintf("tables/volume_interpolations/tryB64factor_%s_%s.csv", channel, kernel),
                         neps=15, zlist=c(0, 1, 2, 3, 4), maxz=5, NDG=5)
      ...
      savename <- sprintf("%s/DM_complete_%s_%s_finvol_%s", savefolder, nameshort[ens_index], channel, kernel)
      result <- read.table(paste0(savename, ".csv"), header=TRUE)
      
      result[, c("DMDq2gev", "dDMDq2gev")] <- result[, c("DMDq2", "dDMDq2")] * conversionfactor
      ...
      for (iz in c(4, 0, 1, 2, 3)) {
        for (err in c("stat", "sys", "vol", "tot")){
          ...
          try(plotwitherror(x=result$q[mask]^2, y=result$DMDq2gev[mask], dy=result$dDMDq2gev[mask],
                            main=paste("differential decay rate for tsink=56, tins=44, sigmoid, cd, ", err, "Z", iz), 
                            xlab="q^2 [GeV^2]", ylab="96 pi^4 DMamma / Dq^2 [GeV^-3]",
                            ylim=c(0, max(result$DMDq2gev[mask] + result$dDMDq2gev[mask])), xlim=c(0, max(result$q[mask]^2))))
          
          try(plotwitherror(x=result$q[mask]^2, y=result$dDMDq2gev
                            [mask],
                            main=paste("differential decay rate for tsink=NA, tins=44, sigmoid, cd, ", err, "Z", iz), 
                            ylab="96 pi^4 d(DMamma / Dq^2) [GeV^-3]", xlim=c(0, max(result$q[mask]^2))))
          
        }
      }
      
      dat <- readRDS(paste0(savename, ".RDS"))
      savename <- sprintf("%s/DM_VEA_epslim_converted_%s_%s_finvol_%s", savefolder, nameshort[ens_index], channel, kernel)
      
      dat$DMDq2gev <- dat$DMDq2 * conversionfactor
      dat$dDMDq2gev <- dat$dDMDq2 * conversionfactor
