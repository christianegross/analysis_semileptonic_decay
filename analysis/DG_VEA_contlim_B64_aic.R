      ...
      for (errtype in errlist) {
        for (iz in zlist) {
          ...
          if(!inherits(fitresult, "try-error")) {
            ...
            if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title), xlab="a[fm^2]", ylab="24 pi^3 DGamma / Dq^2 [GeV^-3]", cols=c("darkgreen", "blue"), drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
            if (doplot) par(lwd=3)
