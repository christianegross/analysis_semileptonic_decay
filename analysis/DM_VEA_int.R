...
pdf("plots/DM_VEA_int.pdf", title="")
...
contlim <- readRDS(sprintf("%s/DM_VEA_contlim_%s_%s_linear.RDS", savefolder, channel, kernel))
...
contlimtable <- read.table(sprintf("%s/DM_VEA_contlim_%s_%s_linear.csv", savefolder, channel, kernel))
...
res <- res[-1, ]
res
write.table(x=res, file=sprintf("%s/DM_VEA_%s_%s_int.csv", savefolder, channel, kernel), row.names=F, col.names=T)

reslist$info <- res
saveRDS(object=reslist, file=sprintf("%s/DM_VEA_%s_%s_int.RDS", savefolder, channel, kernel)) 
