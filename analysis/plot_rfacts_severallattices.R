...
tmp <- read.table(sprintf("%s/%s/%s/m%d_sigmaphys/%s_%.2e/output%s/%s.dat", opt$folder, ensemble, opt$momentum, opt$bmass, opt$kernel, rfacts[1], modefolder, modefolder), col.names=c("mH", "iset", "w", "icomb", "idg", "ieps", "eps", "rho", "stat", "sys", "tot"))
