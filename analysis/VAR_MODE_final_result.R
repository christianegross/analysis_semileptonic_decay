...
if(mode=="DM") {
  zlist <- c(0, 1, 2, 3, 4)
  conversionfactor <- 96*pi^4/m_Ds^3 * m_Ds^1 * G_F^2
...
if(mode=="DM2") {
  zlist <- c(0, 1, 2, 3, 4, 5)
  conversionfactor <- 960*pi^4/m_Ds^3 * m_Ds^2 * G_F^2
...
for (errtype in errlist) {
    ...
    for(iz in zlist) {
        ...
        VAEcomb    <- VAEcomb    + c(VAEcombtab$intspline[VAEcombtab$channel=="cd" & VAEcombtab$iz==iz & VAEcombtab$errtype==errtype], VAEcombdata[[paste0("cd", "combined", errtype, "iz", iz, "bsintspline")]]) * V_cs^2
    }
    if(normalise) {
    ...
    reslist[[paste0(errtype, "VEAerf")]] <- VEAerf
    reslist[[paste0(errtype, "VEAsigmoid")]] <- VEAsigmoid
    reslist[[paste0(errtype, "VAEcomb")]] <- VAEcomb
}
...
write.table(res, sprintf("%s/%s_final_result.csv", savefolder, mode))
saveRDS(res, sprintf("%s/%s_final_result.RDS", savefolder, mode))
warnings()
