...
pdf("plots/DM_VEA_contlim_aic.pdf", title="")

for(kernel in c("sigmoid", "erf")) {
  for(channel in c("cd", "cs")) {
    
    B64 <- readRDS(sprintf("%s/DM_VEA_epslim_converted_B64_%s_%s.RDS", savefolder, channel, kernel))
    C80 <- readRDS(sprintf("%s/DM_VEA_epslim_converted_C80_%s_%s.RDS", savefolder, channel, kernel))
    D96 <- readRDS(sprintf("%s/DM_VEA_epslim_converted_D96_%s_%s.RDS", savefolder, channel, kernel))
    E112 <- readRDS(sprintf("%s/DM_VEA_epslim_converted_E112_%s_%s.RDS", savefolder, channel, kernel))
    
    B64table <- read.table(sprintf("%s/DM_VEA_epslim_converted_B64_%s_%s.csv", savefolder, channel, kernel), header=TRUE)
    C80table <- read.table(sprintf("%s/DM_VEA_epslim_converted_C80_%s_%s.csv", savefolder, channel, kernel), header=TRUE)
    D96table <- read.table(sprintf("%s/DM_VEA_epslim_converted_D96_%s_%s.csv", savefolder, channel, kernel), header=TRUE)
    E112table <- read.table(sprintf("%s/DM_VEA_epslim_converted_E112_%s_%s.csv", savefolder, channel, kernel), header=TRUE)
    ...
    for (th in c(1, 2, 3, 4, 5, 6, 7, 8, 9, 9.5)){
      for (errtype in errlist) {
        for (iz in zlist) {
          ...
          fitresult <- try(combinefunctionsaic(xlist=list(x^2, x^2), ylist=list(y, y),
                                    bsamples=list(bsamples, bsamples), functions=fitfnlist, pars=par.guesslist,
                                    cdf=F, masklist=list(rep(T, 4), rep(T, 4))))
          if(!inherits(fitresult, "try-error")) {
            if (doplot) legendargs=list(x="top", legend=c("linear", "constant"), ncol=2, col=c("darkgreen", "blue"), pch=c(1, 1))
            if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title), xlab="a[fm^2]", ylab="96 pi^4 DGamma / Dq^2 [GeV^-3]", cols=c("darkgreen", "blue"), drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
            if (doplot) par(lwd=3)
            ...
            res <- rbind(res, data.frame(th=th, iz=iz, errtype=errtype, 
                                         lim=fitresult$weighted$mean, dlim=fitresult$weighted$sd, q=B64table$q[abs(B64table$th - th) < 1e-2][1]))
            ...
            if (doplot) try(plotwitherror(x=x, y=y, dy=dy, xlab="a[fm]^2", ylab="96 pi^4 DGamma / Dq^2 [GeV^-3]", main=title))
            res <- rbind(res, data.frame(th=th, iz=iz, errtype=errtype, 
    ...
    res <- res[-1, ]
    write.table(res, sprintf("%s/DM_VEA_contlim_%s_%s_%s.csv", savefolder, channel, kernel, comment))
    saveRDS(reslist, sprintf("%s/DM_VEA_contlim_%s_%s_%s.RDS", savefolder, channel, kernel, comment))
    
    
    
    res <- read.table(sprintf("%s/DM_VEA_contlim_%s_%s_%s.csv", savefolder, channel, kernel, comment), header=TRUE)
    for (errtype in errlist) {
      plot(NA,
      ...
      plotwitherror(x=res$q[mask]^2, y=dqsum, dy=sqrt(ddqsum2),
                    ...
                    xlab=expression(paste(q^2, " [", GeV^2, "]", sep="")),
                    ylab="", col=5)
      mtext(side=2, text=expression(paste("96", pi^4, frac(D*Gamma , D*q^2),  "[", GeV^-3,"]")), line=1.5)
      for (iz in c(0, 1, 2, 3)) {
        mask <- res$iz==iz & res$errtype==errtype
        plotwitherror(x=res$q[mask]^2, y=res$lim[mask], dy=res$dlim[mask], 
                      rep=TRUE, col=iz+1)
      ...
      legend(x = "topleft", 
             legend=c(0, 1, 2, 3, "sum"), col=seq(1, 5), pch=rep(1, 5), title="Z")
    }
  }
}
