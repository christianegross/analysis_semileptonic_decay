...
if(mode=="DG") {
    zlist <- c(0, 1, 2, 3)
    ylab <- "48 pi^5 DG/Dw^2"
}

if(mode=="DM") {
    zlist <- c(0, 1, 2, 3, 4)
    ylab <- "96 pi^5 DM/Dw^2"
}

if(mode=="DM2") {
    zlist <- c(0, 1, 2, 3, 4, 5)
    ylab <- "960 pi^5 DM2/Dq^2"
}
...
pdf(sprintf("plots/%s_VEA_contlim_aic.pdf", mode), title="")
...
for(kernel in c("sigmoid", "erf")) {
  for(channel in c("cd", "cs")) {
    ...
    C80 <- readRDS(sprintf("%s/%s_VEA_epslim_C80_%s_%s.RDS", savefolder, mode, channel, kernel))
    D96 <- readRDS(sprintf("%s/%s_VEA_epslim_D96_%s_%s.RDS", savefolder, mode, channel, kernel))
    E112 <- readRDS(sprintf("%s/%s_VEA_epslim_E112_%s_%s.RDS", savefolder, mode, channel, kernel))
    
    B64table <- read.table(sprintf("%s/%s_VEA_epslim_converted_B64_%s_%s.csv", savefolder, mode, channel, kernel), header=TRUE)
    C80table <- read.table(sprintf("%s/%s_VEA_epslim_converted_C80_%s_%s.csv", savefolder, mode, channel, kernel), header=TRUE)
    D96table <- read.table(sprintf("%s/%s_VEA_epslim_converted_D96_%s_%s.csv", savefolder, mode, channel, kernel), header=TRUE)
    E112table <- read.table(sprintf("%s/%s_VEA_epslim_converted_E112_%s_%s.csv", savefolder, mode, channel, kernel), header=TRUE)
        ...
        for (iz in zlist) {
          ...
          for (index in seq(1, 4)) {
            if(!is.na(tablelist[[index]]$q[abs(tablelist[[index]]$th - th) < 1e-2][1]^2)) {
              ...
              try(bsamples[, index] <- enslist[[index]]$dat[, mask])
          ...
          if(!inherits(fitresult, "try-error")) {
            ...
            if (doplot) try(plotlistfits(fitresult$fitresults, main=paste(title), xlab="a[fm^2]", ylab=ylab, cols=c("darkgreen", "blue"), drawlegend=T, legendargs=legendargs, xlim=c(0, 0.0065)))
            if (doplot) par(lwd=3)
            ...
            if (doplot) try(plotwitherror(x=x, y=y, dy=dy, xlab="a[fm]^2", ylab=ylab, main=title))
            res <- rbind(res, data.frame(th=th, iz=iz, errtype=errtype, 
    ...
    res <- res[-1, ]
    write.table(res, sprintf("%s/%s_VEA_contlim_%s_%s_%s.csv", savefolder, mode, channel, kernel, comment))
    saveRDS(reslist, sprintf("%s/%s_VEA_contlim_%s_%s_%s.RDS", savefolder, mode, channel, kernel, comment))
    
    
    
    res <- read.table(sprintf("%s/%s_VEA_contlim_%s_%s_%s.csv", savefolder, mode, channel, kernel, comment), header=TRUE)
    for (errtype in errlist) {
      plot(NA,
