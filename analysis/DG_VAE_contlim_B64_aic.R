...
#~ for(channel in c("cs")) {
#~     for(kernel in c("sigmoid")) {
#~ for(theta in as.character(c(4))) {
  pdf(sprintf("plots/DG_%s_%s_VAE_contlim_th%s%s_aic.pdf", channel, kernel, theta, comment),)
  savename <- sprintf("%s/DG_VAE_contlim_%s_%s_th%s_aic", savefolder, channel, kernel, theta)
  ...
  mine <- determineDGDq2_contlim_aic(resultpathlist=list(sprintf("/hiskp4/gross/heavymesons/data/%s/cB211.07.64/th%s/", channel, theta),
                                 ...
                                 volumetablelist = c(sprintf("tables/volume_interpolations/DG_tryB64factor_%s_%s.csv", channel, kernel), 
