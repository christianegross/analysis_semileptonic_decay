...
mytable <- read.table(sprintf("tables/DG_VAE_epslim%s.csv", comment), header=TRUE)
mydata <- readRDS(sprintf("tables/DG_VAE_epslim%s.RDS", comment))
...
res <- res[-1, ]
res
write.table(x=res, file=sprintf("tables/DG_VAE_int%s.csv", comment), row.names=F, col.names=T)

reslist$info <- res
saveRDS(object=reslist, file=sprintf("tables/DG_VAE_int%s.RDS", comment)) 
