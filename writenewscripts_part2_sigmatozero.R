...
fileDG <- sprintf("%s/%s-extrapoletesigma-DGDq2.Rmd", parent, 4*length(folders)+2)
fileDM <- sprintf("%s/%s-extrapoletesigma-DMDq2.Rmd", parent, 4*length(folders)+2)
cat(
"---",
"output:",
...
"maxAmin <- 1",
"",
sprintf("files <- c(\"th1/%s_new/outputDGammaDq2/DGammaDq2.bin\",", kernel),
...
#~             "  for (err in c(\"stat\", \"sys\")){",
            ...
            "                  [mask],",
            sprintf("                  main=paste(\"differential decay rate for tsink=%d, tins=%d, %s, %s, \", err, \"Z\", iz), ", ensinfos["tsnk", ], ensinfos["tj", ], kernel, channel),
...
"maxAmin <- 1",
"",
sprintf("files <- c(\"th1/%s_new/outputDMDq2/DMDq2.bin\",", kernel),
...
file=fileDM, append=TRUE, sep="\n")
        for (i in seq_along(folders)) {
            ...
            sprintf("Nt <- rep(%d, 10)", ensinfos["Nt", ]),
            "",
            sprintf("savename <- sprintf(\"tables/DM_complete_c%s_%s_finvol_%s\")", nameshort[i], channel, kernel),
            "",
            "par.guess <- rep(1, 5)",
            "",
            "",
...
#~             "        fitfn = fneight, par.guess = par.guess, errors=c(\"stat\", \"sys\", \"vol\", \"tot\"),",
            "        fitfn = fneight, par.guess = par.guess, errors=c(\"stat\", \"sys\"),",
            "        zlist=c(0, 1, 2, 3, 4), NDG=5, maxz=5,",
            sprintf("        volumetable=\"tables/finitevolume_B_%s_%s.csv\", doplot=!(knitr::is_html_output()),", channel, kernel),
            "        neps=19)",
            "",
            "```",
            "",
...
#~             "  for (err in c(\"stat\", \"sys\", \"vol\", \"tot\")){",
            ...
            "    mask <- result$iz==iz & result$icomb==0 & result$errtype==err",
            "    try(plotwitherror(x=result$q[mask]^2, y=result$DGDq2gev[mask], dy=result$dDGDq2gev[mask],",
            sprintf("                  main=paste(\"differential decay rate for tsink=%d, tins=%d, %s, %s, \", err), ", ensinfos["tsink", ], ensinfos["tj", ], kernel, channel),
            "                  xlab=\"q^2 [GeV^2]\", ylab=\"24 pi^3 DGamma / Dq^2 [GeV^-3]\",",
            "                  ylim=c(0, max(result$DGDq2gev[mask] + result$dDGDq2gev[mask])), xlim=c(0, max(result$q[mask]^2))))",
            "    ",
            "    try(plotwitherror(x=result$q[mask]^2, y=result$dDGDq2gev",
            "                  [mask],",
            sprintf("                  main=paste(\"differential decay rate for tsink=%d, tins=%d, %s, %s, \", err), ", ensinfos["tsnk", ], ensinfos["tj", ], kernel, channel),
            "                  ylab=\"24 pi^3 d(DGamma / Dq^2) [GeV^-3]\", xlim=c(0, max(result$q[mask]^2))))",
            "",
            "  }",
            "}",
            "",
            "dat <- readRDS(paste0(savename, \".RDS\"))",
            sprintf("savename <- sprintf(\"tables/DM_complete_c%s_converted_%s_finvol_%s\")", nameshort[i], channel, kernel),
            "",
            "dat$DGDq2gev <- dat$DGDq2 * conversionfactor",
            "dat$dDGDq2gev <- dat$dDGDq2 * conversionfactor",
            "dat$datgev <- dat$dat * conversionfactor",
...
sprintf("```{r decayrate%sfinvol_%s}", channel, kernel), 
"", 
sprintf("cB64 <- read.table(\"tables/DM_complete_cB64_converted_%s_finvol_%s.csv\")", channel, kernel), 
sprintf("cB96 <- read.table(\"tables/DM_complete_cB96_converted_%s_finvol_%s.csv\")", channel, kernel), 
sprintf("cC80 <- read.table(\"tables/DM_complete_cC80_converted_%s_finvol_%s.csv\")", channel, kernel), 
sprintf("cD96 <- read.table(\"tables/DM_complete_cD96_converted_%s_finvol_%s.csv\")", channel, kernel), 
sprintf("cE112 <- read.table(\"tables/DM_complete_cE112_converted_%s_finvol_%s.csv\")", channel, kernel), 
sprintf("cB48 <- read.table(\"tables/DM_complete_cB48_400_converted_%s_finvol_%s.csv\")", channel, kernel), 
"", 
"cB64 <- na.omit(cB64)", 
"cB96 <- na.omit(cB96)", 
"cC80 <- na.omit(cC80)", 
