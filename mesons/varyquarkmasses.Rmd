...
ensinfos <- data.frame(ens=c("cB211.07.48_novariation", "cB211.07.48_vary", 
                             "cB211.07.48_strange", "cB211.07.48_charm", 
                             "cB211.07.48_charm_strange"), 
                       name=c("B48_static", "B48_vary_masses", "B48_strange", "B48_charm", "B48_charm_strange"), 
                       ...
                       amu_s = c(0.01692, 0.01692, 0.019, 0.01692, 0.019), 
                       amu_c = c(0.2368, 0.2368, 0.2368, 0.26, 0.26), 
                       L=c(48, 48, 48, 48, 48), 
                       B_ens=c(T, T, T, T, T))
ensinfos <- data.frame(ens=c("cB211.07.48_novariation", "cB211.07.48_vary"), 
                       name=c("B48_static", "B48_vary_masses"), 
                       afm=c(0.07957, 0.07957), 
                       uplim=c(5, 5), lowlim=c(10, 10), 
                       kappa=c(0.1394265, 0.1394265),
                       amu_d = c(0.00072, 0.000722), 
                       amu_s = c(0.01692, 0.01692), 
                       amu_c = c(0.2368, 0.2368), 
                       L=c(48, 48), 
                       B_ens=c(T, T))
...
write.table(x=res, file="calculatedmesonmasses_vary_quark_mass.csv", row.names=FALSE)
saveRDS(object=reslist, file="calculatedmesonmasses_vary_quark_mass.RDS")
# normdist

```
```{r, eval=F}
pdf("plotsnormaldistribution/pvaluesnormaldistribution.pdf", title="")
for (ens in unique(tab$ensemble)) {
  for (name in c("Ds", "D", "eta", "K", "pi", "Ds_sl", "D_sl", "eta_sl", "K_sl", "pi_sl")) {
    mask <- normdist$ens==ens & normdist$name==name
    plot(x=normdist$time[mask], y=normdist$pcorr[mask], main=paste(ens, name, "correlator"), xlab="time", ylab="p-value")
    plot(x=normdist$time[mask], y=normdist$pmass[mask], main=paste(ens, name, "mass"), xlab="time", ylab="p-value")
  }
}
```
...
```{r}
print(res$name[1:10])
plotwitherror(x=seq(1, 10), y=res$amass[11:20]/res$amass[1:10], 
              dy=sqrt((res$damass[11:20]/res$amass[1:10])^2 +
                        (res$amass[11:20]*res$damass[1:10]/res$amass[1:10]^2)^2), 
              xlab="meson index", ylab="ratio mass", main="ratio meson mass vary/static")
lines(x=c(-100, 100), y=c(1, 1), col="red")
plotwitherror(x=seq(1, 10), y=res$damass[11:20]/res$damass[1:10], 
              xlab="meson index", main="ratio errors meson mass vary/static", ylab="ratio error")
lines(x=c(-100, 100), y=c(1, 1), col="red")
```
...
```{r masscombined}
res <- read.table("calculatedmesonmasses_vary_quark_mass.csv", header=T)
reslist <- readRDS("calculatedmesonmasses_vary_quark_mass.RDS")
...
for (name_id in seq_along(names)) {
  ...
  plotwitherror(x=reslist[[name]][[1]]$t.idx, 
                y=reslist[[name]][[1]]$deffMass,
                xlab="t/a", ylab="am_eff ", main=paste(name, "sd"), col=colors[1], pch=colors[1],
                xlim=c(10, 48))
  for (i in seq(2, length(reslist$info$name))) {
    plotwitherror(x=reslist[[name]][[i]]$t.idx, 
                  y=reslist[[name]][[i]]$deffMass,
                  pch=colors[i], col=colors[i], rep=TRUE)
  }
  legend(x="topleft", legend=reslist$info$name, col=colors, pch=colors)
  print("")
  
  
  plotwitherror(x=reslist[[name]][[1]]$t.idx, 
                y=reslist[[name]][[2]]$effMass/reslist[[name]][[1]]$effMass,
                dy=apply(X=reslist[[name]][[2]]$effMass.tsboot/reslist[[name]][[1]]$effMass.tsboot,
                         FUN=sd, MARGIN=2),
                xlab="t/a", ylab="ratio am_eff ", main=paste(name, "ratio mean vary/static"), col=colors[1], pch=colors[1],
                xlim=c(10, 48))
  lines(x=c(-100, 1000), y=c(1, 1), col="red")
  print("")  
  
  plotwitherror(x=reslist[[name]][[1]]$t.idx, 
                y=(reslist[[name]][[2]]$effMass-reslist[[name]][[1]]$effMass)/reslist[[name]][[1]]$effMass,
                dy=apply(X=(reslist[[name]][[2]]$effMass.tsboot-reslist[[name]][[1]]$effMass.tsboot)/reslist[[name]][[1]]$effMass.tsboot,
                         FUN=sd, MARGIN=2),
                xlab="t/a", ylab="ratio am_eff ", main=paste(name, "relative deviation vary/static"), col=colors[1], pch=colors[1],
                xlim=c(10, 48))
  lines(x=c(-100, 1000), y=c(1, 1), col="red")
  print("")
  ...
  plotwitherror(x=reslist[[name]][[1]]$t.idx, 
                y=reslist[[name]][[2]]$deffMass/reslist[[name]][[1]]$deffMass,
                xlab="t/a", ylab="ratio am_eff ", main=paste(name, "ratio sd vary/static"), col=colors[1], pch=colors[1],
                xlim=c(10, 48))
  lines(x=c(-100, 1000), y=c(1, 1), col="red")
  print("")
