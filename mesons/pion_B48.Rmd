...
boot.l <- 5
afmB48 <- 0.07957 #fm
amevB48m1 <- 197.3269804/afmB48
amevB48 <- afmB48/197.3269804
```
```{r, eval=T}
pi_ss <- readcffrompreprocessed(realfile="pi.dat", path="/home/gross/Documents/heavymesons/data/meson/cB211.07.48/4Pts_meson_data/")
pi_sl <- readcffrompreprocessed(realfile="pi_sl.dat", path="/home/gross/Documents/heavymesons/data/meson/cB211.07.48/4Pts_meson_data/")
...
saveRDS(pi_ss_aic, file="pi_ss_aic_B48.RDS")
saveRDS(pi_sl_aic, file="pi_sl_aic_B48.RDS")
saveRDS(pi_ss_mass, file="pi_ss_mass_B48.RDS")
saveRDS(pi_sl_mass, file="pi_sl_mass_B48.RDS")
```

## Mass determination

We determine the mass of the pion in the B48 lattice by using the aic procedure on effectivemass-fits.

The masses of the valence quarks was $a\mu_{d}=0.00072$, we have measured the two-point functions of the smeared-smeared and local-smeared correlator. 
We have used 300 configurations, from 2 different replicas.
...
We assume the lattice spacing to be `r afmB48` fm, as in the tau decay paper.

```{r}
pi_ss_aic <- readRDS("pi_ss_aic_B48.RDS")
pi_sl_aic <- readRDS("pi_sl_aic_B48.RDS")
pi_ss_mass <- readRDS("pi_ss_mass_B48.RDS")
pi_sl_mass <- readRDS("pi_sl_mass_B48.RDS")
afmB48 <- 0.07957 #fm
amevB48m1 <- 197.3269804/afmB48
...
plotwitherror(x=pi_ss_mass$t.idx, 
              y=pi_ss_mass$effMass*amevB48m1, 
              dy=pi_ss_mass$deffMass*amevB48m1, 
     xlab="t/a", ylab="m [MeV]", 
     main="smeared-smeaered pion",
     ylim=(pi_ss_aic$effmassfit$t0 + c(-1, 1)*3*pi_ss_aic$effmassfit$se)*amevB48m1)
...
polygon(x=c(-100, 200, 200, -100),
        y=(pi_ss_aic$effmassfit$t0+pi_ss_aic$effmassfit$se*c(-1, -1, 1, 1))*amevB48m1, 
        col=pcol1, lty = 0, lwd = 0.001, border = pcol1)
polygon(x=c(-100, 200, 200, -100),
        y=(pi_ss_aic$effmassfit$t0+sd(pi_ss_aic$boot$m50)*c(-1, -1, 1, 1))*amevB48m1, 
        col=pcol2, lty = 0, lwd = 0.001, border = pcol2)
plotwitherror(x=pi_ss_mass$t.idx, y=pi_ss_mass$effMass*amevB48m1, dy=pi_ss_mass$deffMass*amevB48m1, rep=TRUE)
lines(x=c(-100, 200), y=rep(pi_ss_aic$effmassfit$t0*amevB48m1, 2), lwd=2, lty=2)
...
plotwitherror(x=pi_sl_mass$t.idx, 
              y=pi_sl_mass$effMass*amevB48m1, 
              dy=pi_sl_mass$deffMass*amevB48m1, 
     xlab="t/a", ylab="m [MeV]", 
     main="smeared-local pion",
     ylim=(pi_sl_aic$effmassfit$t0 + c(-1, 1)*3*pi_sl_aic$effmassfit$se)*amevB48m1)
pcol1 <- col2rgb("red", alpha = TRUE) / 255
pcol1[4] <- 0.65
pcol1 <- rgb(red = pcol1[1], green = pcol1[2],
             blue = pcol1[3], alpha = pcol1[4])
...
pcol2 <- rgb(red = pcol2[1], green = pcol2[2],
            blue = pcol2[3], alpha = pcol2[4])
polygon(x=c(-100, 200, 200, -100),
        y=(pi_sl_aic$effmassfit$t0+pi_sl_aic$effmassfit$se*c(-1, -1, 1, 1))*amevB48m1, 
        col=pcol1, lty = 0, lwd = 0.001, border = pcol1)
polygon(x=c(-100, 200, 200, -100),
        y=(pi_sl_aic$effmassfit$t0+sd(pi_sl_aic$boot$m50)*c(-1, -1, 1, 1))*amevB48m1, 
        col=pcol2, lty = 0, lwd = 0.001, border = pcol2)
plotwitherror(x=pi_sl_mass$t.idx, y=pi_sl_mass$effMass*amevB48m1, dy=pi_sl_mass$deffMass*amevB48m1, rep=TRUE)
lines(x=c(-100, 200), y=rep(pi_sl_aic$effmassfit$t0*amevB48m1, 2), lwd=2, lty=2)
legend(x="topright", legend=c("meas", "comb", "stat"), 
       fill=c(NA, pcol1, pcol2), border=c(NA, pcol1, pcol2), 
       pch=c(1, NA, NA), co=c(1, NA, NA))
...
res$massmev <- res$amass*amevB48m1
res$dmassstatmev <- res$damassstat*amevB48m1
res$dmasscombmev <- res$damasscomb*amevB48m1

res$dmasssysmev <- sqrt(res$dmasscombmev^2-res$dmassstatmev^2)
knitr::kable((res))
write.table(x=res, file="resultspiB48.csv", row.names=F)
```
```{r}
res <- read.table("resultspiB48.csv", header=T)
# res
afm <- 0.07957
dafm <- 0.00013
...
The B-lattice spacing is given as `r tex.catwitherror(afm, dafm, with.dollar=F, with.cdot=F, digits=2)`, meaning the relative error is `r format(dafm/afm, digits=3)`. 
