...
Hmass.matrixfit <- matrixfit(cf=Hmass, t1 = as.numeric(parameters$V2[parameters$V1 == "tfith"]), t2 = as.numeric(parameters$V3[parameters$V1 == "tfith"]), sym.vec=c("cosh"), useCov = TRUE)
summary(Hmass.matrixfit)
...
characteristics$w <- characteristics$w / Hmass.matrixfit$t0[1]
...
time <- seq_along(unsymmetrise.cf(Hmass)$cf0)-1
tmp <- unsymmetrise.cf(Hmass)
ret <- tmp

# use cf_boot because otherwise the error is not estimated correctly for the jackknife
ret$cf.tsboot$t <- 0.5*Hmass.matrixfit$t[, 2]^2 * (exp(-outer(Hmass.matrixfit$t[, 1], time)) + exp(-outer(Hmass.matrixfit$t[, 1], (length(time)-time)))) * exp((tj - tsink) * Hmass.matrixfit$t[, 1]) / Hmass.matrixfit$t[, 1]
ret$cf.tsboot$t0 <- 0.5*Hmass.matrixfit$t0[2]^2 * (exp((-time) * Hmass.matrixfit$t0[1]) + exp(-(length(time)-time) * Hmass.matrixfit$t0[1])) * exp((tj - tsink) * Hmass.matrixfit$t0[1]) / Hmass.matrixfit$t0[1]
...
store_bin_cf_effmass_matrixfit(to.write, Hmass.matrixfit,   endian)
...
for(index in 1:5) {
    yres <- data.frame(mass=Hmass.matrixfit$t0[1], dmass=Hmass.matrixfit$se[1], iset=0, w=characteristics$w, iy=index, t=(0:(tj)))
    for (comb in c("FULL", "VPAR", "APAR", "VPERP", "APERP")) {
