...
parameters <- read.table(args[1], fill = TRUE, col.names = paste0("V", 1:4), colClasses=c("character"))
print(parameters)
...
if(as.character(parameters$V2[parameters$V1 == "tfith"]) == "read") {
    print(as.character(parameters$V3[parameters$V1 == "tfith"]))
    emass <- readRDS(as.character(parameters$V3[parameters$V1 == "tfith"]))
} else {
...
if(as.character(parameters$V2[parameters$V1 == "tfitl"]) == "read") {
    Lemass <- readRDS(as.character(parameters$V3[parameters$V1 == "tfitl"]))
} else{
...
#~ print(summary(Lemass/emass))

print(summary(Lemass))
}

print(paste("## Ratio mass 2 and mass 1:", 
            tex.catwitherror(x=Lemass$effmassfit$t0[1]/emass$effmassfit$t0[1], dx=sd(Lemass$effmassfit$t[,1]/emass$effmassfit$t[,1]), digits=2, with.cdot=F, with.dollar=F)))
            
print("## physical ratio M_Ds/M_Bs=0.366755296")    

## determine mass heavy
L0mass <- readcffrompreprocessed(realfile = paste(sep = "/", parameters$V2[parameters$V1 == "path"], parameters$V2[parameters$V1 == "L0file"]), symmetrise = T)
...
if(as.character(parameters$V2[parameters$V1 == "tfitl0"]) == "read") {
    L0emass <- readRDS(as.character(parameters$V3[parameters$V1 == "tfitl0"]))
} else {
...
parameters$V2[parameters$V1=="path"] <- "."
print(parameters)
...
ret <- tmp
#~ print(tsink-tj+1)
ret$cf.tsboot$t <- tmp$cf.tsboot$t[, rep(tsink-tj+1, as.numeric(parameters$V2[parameters$V1=="T"]))] * exp((tj - tsink) * emass$effmassfit$t[, 1]) / emass$effmassfit$t[, 1]
ret$cf.tsboot$t0 <- tmp$cf.tsboot$t0[rep(tsink-tj+1, as.numeric(parameters$V2[parameters$V1=="T"]))] * exp((tj - tsink) * emass$effmassfit$t0[1]) / emass$effmassfit$t0[1]
...
setcorrecttimeindex <- function(y, tj) {
    tmp <- y
    y$cf.tsboot$t[, (0:(tj)) + 1] <- tmp$cf.tsboot$t[, (0:(tj)) + 1 + tsink - tj]
    y$cf.tsboot$t0[(0:(tj)) + 1] <- tmp$cf.tsboot$t0[(0:(tj)) + 1 + tsink - tj]
